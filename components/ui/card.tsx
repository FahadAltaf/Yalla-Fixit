...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "border-border bg-card text-card-foreground gap-4 overflow-hidden rounded-lg py-4 text-sm border shadow-[0_1px_2px_rgba(22,24,26,0.06)] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:gap-3 data-[size=sm]:py-3 data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-lg *:[img:last-child]:rounded-b-lg group/card flex flex-col",
        className
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        " leading-snug font-medium group-data-[size=sm]/card:text-sm text-lg sm:text-xl",
        className
      )}
      {...props}
    />
