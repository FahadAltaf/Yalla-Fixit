...
>(
  (
    ...
    const AllSelectedItem = React.useCallback(() => {
      // Check if all available options are selected
      const totalOptions = Object.values(options).flat().length;
      const selectablesCount = Object.values(selectables).flat().length;

      // Only show if there were options to begin with, all are selected, and not creatable
      if (totalOptions > 0 && selectablesCount === 0 && !creatable) {
        return (
                  ...
                  <>
                    ...
                    {Object.entries(selectables).map(([key, dropdowns]) => (
                      <CommandGroup
                        key={key}
                        heading={key}
                        className="max-h-[170px] overflow-auto"
                      >
                        <>
