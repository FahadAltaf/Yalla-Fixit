...
function Stat({
  ...
  return (
    ...
    >
      <div style={{ fontSize: 19, fontWeight: 800, lineHeight: 1.1, color: t ? t.fg : C.ink }}>
        {value}
      </div>
      <div
        style={{
...
export const InspectionReport = forwardRef<HTMLDivElement, {
  ...
  const propertyLine = [property?.building_name, property?.community, property?.city]
    .filter(Boolean)
    .join(", ");
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
          ...
          <img
            src={yallaFixit.src}
            alt="Yalla Fixit"
            style={{ width: 50, height: 50, objectFit: "contain", objectPosition: "left" }}
          />
          <div>
            <div style={{ fontSize: 15, fontWeight: 800, color: C.ink, letterSpacing: -0.3 }}>
              Yalla Fix It
            </div>
            <div style={{ fontSize: 8, color: C.sub, lineHeight: 1.5, marginTop: 2 }}>
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <div style={{ fontSize: 10, fontWeight: 700, color: C.ink, marginTop: 4 }}>
            {task.code}
          </div>
      ...
      <div style={{ height: 2, background: C.brand, borderRadius: 2, margin: "6px 0 8px" }} />
      ...
      <div style={{ display: "flex", gap: 10 }}>
        <Card style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 12, fontWeight: 700, color: C.ink, marginTop: 3 }}>
            {property?.unit_label || task.code}
          </div>
          <div style={{ fontSize: 9.5, color: C.sub, marginTop: 2 }}>{propertyLine || "—"}</div>
        ...
        <Card style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 9.5, color: C.sub }}>{property?.client_phone || ""}</div>
        </Card>
      </div>
      ...
      >
        ...
        <span style={{ flex: 1 }}>
          ...
          <span style={{ fontWeight: 600 }}>{inspector?.full_name || "—"}</span>
        </span>
      </div>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <Stat label="High" value={String(high)} tone={high > 0 ? "high" : undefined} />
        <Stat label="Medium" value={String(medium)} tone={medium > 0 ? "medium" : undefined} />
        <Stat label="Low" value={String(low)} tone={low > 0 ? "low" : undefined} />
        <Stat label="Areas walked" value={`${confirmedAreas}/${areas.length}`} />
        <Stat label="Checklist" value={`${checklistDone}/${checklist.length}`} />
      </div>
          ...
          <Card style={{ padding: "6px 14px" }}>
            {accessIssues.map((area, index) => (
              ...
              >
                ...
                <span style={{ fontWeight: 600, fontSize: 10, color: C.ink }}>{area.name}</span>
                {area.access_reason ? (
                  <span style={{ color: C.sub, fontSize: 9.5 }}>— {area.access_reason}</span>
                ) : null}
              </div>
            ))}
          </Card>
      ...
      {areas.length === 0 ? (
        ...
        areas.map((area) => {
          ...
          return (
            <Card
              key={area.id}
              style={{ padding: 0, marginBottom: 5, breakInside: "avoid", background: "#ffffff" }}
            >
              <div
                style={{
                  display: "flex",
                  ...
                  alignItems: "center",
                  background: C.card,
                  borderBottom: areaSnags.length > 0 ? `1px solid ${C.line}` : "none",
                  borderRadius: areaSnags.length > 0 ? "8px 8px 0 0" : 8,
                  padding: "6px 14px",
                }}
              >
                <span style={{ fontWeight: 700, fontSize: 10.5, color: C.ink }}>{area.name}</span>
                  ...
                  >
                    ...
                    >
                      <div style={{ flex: 1 }}>
                        ...
                        {snag.note ? (
                          <div style={{ color: C.sub, fontSize: 9.5, marginTop: 1 }}>
                            {snag.note}
                          </div>
                        ) : null}
                        {photos[0]?.taken_at ? (
                      ...
                      <Pill tone={(SEVERITY[snag.severity] ?? SEVERITY.low).tone}>
      ...
      {quotation ? (
        <>
          ...
          <Card style={{ padding: "8px 14px", breakInside: "avoid" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 9.5 }}>
              <tbody>
                {quotation.lines.map((line, i) => (
                  <tr key={i} style={{ borderBottom: `1px solid ${C.line}` }}>
                    <td style={{ padding: "4px 0" }}>{line.description}</td>
                    <td style={{ padding: "4px 0", textAlign: "right", color: C.sub }}>
                      {line.qty} {line.unit}
                    </td>
                ...
                <tr>
                  <td style={{ padding: "4px 0", textAlign: "right", color: C.sub }} colSpan={2}>
                    Subtotal
                  </td>
                ...
                <tr style={{ borderTop: `1.5px solid ${C.ink}` }}>
                  <td
                    style={{ padding: "6px 0", textAlign: "right", fontWeight: 800, fontSize: 10.5 }}
                    colSpan={2}
                  >
                    Total
                  </td>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 20 }}>
          ...
          <div style={{ textAlign: "right" }}>
            {submission?.signature_url ? (
              ...
              <span style={{ fontFamily: SCRIPT, fontSize: 22, color: C.ink }}>
                {submission.signer_name}
              </span>
            ) : null}
            <div
      ...
      >
        This report records the condition observed at the time of inspection. Defects are classified
        by severity for prioritisation and do not constitute a structural or legal certification.
        Yalla Fixit Property Care.
      </div>
    </div>
  );
});
...
function ChecklistBlock({ items }: { items: SnaggingChecklistItem[] }) {
  ...
  for (const entry of entries) {
    let target = 0;
    for (let i = 1; i < columnCount; i++) if (load[i] < load[target]) target = i;
            ...
            >
              ...
              {groupItems.map((item, index) => {
                ...
                return (
                  ...
                  >
                    <span style={{ fontSize: 9, color: C.body }}>{item.label}</span>
